...
/**
 * Returns true when access is held (badge cleared), false when it warned.
 *
 * Unable to tell → treated as held. Crying wolf on a working install is worse
...
export async function refreshPermissionBadge(
  ...
  { origins, alertTitle }: PermissionBadgeOptions,
): Promise<boolean> {
  if (origins.length === 0) return true;
  const held = await permissions.contains({ origins }).catch(() => true);
